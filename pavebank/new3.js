...
class LineGrid {
  ...
  _tick(ts) {
    ...
    this._prevTs  = this.lastTs;  // save before overwrite for productTime delta
    this.lastTs   = ts;
    this.angle   += 0.009;
    ...
    // Product visual switcher — productTime is the sole clock for all product animations
    if (this.productVisual) {
      const dtProduct = this.lastTs !== null ? (ts - this._prevTs) / 1000 : 0;
